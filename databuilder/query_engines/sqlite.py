...
class SQLiteQueryEngine(BaseQueryEngine):
    ...
    def select_patient_id_for_population(self, population_expression):
        tables = remove_duplicates(population_expression._from_obj)
        if len(tables) > 1:
...
def remove_duplicates(elements):
    "Remove duplicates from elements while preserving order"
    return list(dict.from_keys(elements).keys())
