...
def test_in_filter_on_query_values(database, setup_test_database, mock_backend):
    ...
    class Cohort:
        ...
        _first_code1_event_on_positive_test_date = (
            table("clinical_events")
            .filter(code="Code1")
            .filter("date", is_in=positive_test_dates)
        ).earliest()
        date = _first_code1_event_on_positive_test_date.get("date")
        value = _first_code1_event_on_positive_test_date.get("result")
        excludes = ["positive_test_dates"]
...
def test_not_in_filter_on_query_values(database, setup_test_database, mock_backend):
    ...
    class Cohort:
        ...
        _first_code1_events_not_on_test_date = (
            table("clinical_events").filter("date", not_in=test_dates).earliest()
        )
        date = _first_code1_events_not_on_test_date.get("date")
        value = _first_code1_events_not_on_test_date.get("result")
        excludes = ["test_dates"]
