...
def test_run_generated_sql_get_single_column_default_population(
    ...
    input_data = [
        *patient(1, event("Code1")),
        # patient 2 has an event, but no RegistrationHistory entry
        Events(PatientId=2, EventCode="Code2", System="ctv3"),
    ]
...
def test_run_generated_sql_get_multiple_columns(database, setup_test_database):
    input_data = [
        *patient(1, event("Code1"), positive_test(True)),
        *patient(2, event("Code2"), positive_test(False)),
    ]
    setup_test_database(input_data)
...
def test_run_generated_sql_get_single_row_per_patient(
    ...
    input_data = [
        *patient(
            1,
            event("Code1", "2021-01-03"),
            event("Code1", "2021-02-01"),
            event("Code2", "2021-05-02"),
        ),
        *patient(2, event("Code1", "2021-06-05"), event("Code1", "2021-02-04")),
    ]
    setup_test_database(input_data)
...
def test_filter_between_other_query_values(database, setup_test_database):
    ...
    input_data = [
        *patient(
            ...
            event("Code1", "2021-02-01", system="ctv3", value=10.1),
            event("Code1", "2021-04-12", system="ctv3", value=10.2),
            event("Code1", "2021-03-01", system="ctv3", value=10.3),
        ),
        *patient(
            2,
            ...
            event("Code1", "2021-01-10", system="ctv3", value=50.1),
            event("Code1", "2021-02-01", system="ctv3", value=50.2),
            event("Code1", "2021-05-02", system="ctv3", value=50.3),
        ),
        *patient(
            3,
            ...
            event("Code1", "2021-03-15", system="ctv3", value=60.1),
            event("Code1", "2021-04-01", system="ctv3", value=60.2),
            # within dates, but different code
            event("Code2", "2021-02-01", system="ctv3", value=60.3),
            # within dates, but different system
            event("Code2", "2021-02-01", system="snomed", value=60.3),
        ),
    ]
    setup_test_database(input_data)
...
def test_in_filter_on_query_values(database, setup_test_database):
    ...
    input_data = [
        *patient(
            ...
            event("Code1", "2021-01-01", value=10.1),
            event("Code1", "2021-02-15", value=10.2),
            # 1 result that doesn't match a positive result date
            event("Code1", "2021-05-01", value=10.3),
        ),
        *patient(
            2,
            positive_test(True, "2021-01-10"),
...
def test_not_in_filter_on_query_values(database, setup_test_database):
    ...
    input_data = [
        *patient(
            ...
            event("Code1", "2021-04-01", value=10.3),
        ),
        *patient(
            2,
            positive_test(True, "2021-01-10"),
            ...
            event("Code1", "2021-05-02", value=50.3),
        ),
    ]
...
def test_aggregation(database, setup_test_database, aggregation, column, expected):
    input_data = [
        *patient(
            1,
            event("Code1", value=10.1),
            event("Code1", value=10.5),
            event("Code2", value=30.1),
        ),
        *patient(
            2,
            event("Code1", value=50.1),
            event("Code2", value=60.1),
        ),
        *patient(
            3,
            event("Code2", value=70.1),
        ),
    ]
    setup_test_database(input_data)
...
def test_categorise_multiple_values(database, setup_test_database):
    ...
    input_data = [
        *patient(1, event("abc"), height=200),
        *patient(2, event("xyz"), height=150),
        *patient(3, event("abc"), height=160),
    ]
    setup_test_database(input_data)
...
def test_categorise_nested_comparisons(database, setup_test_database):
    input_data = [
        *patient(1, event("abc"), height=194),  # tall with code - matches
        *patient(2, event("xyz"), height=200.5),  # tall no code  - matches
        *patient(3, event("abc"), height=140.5),  # short with code - matches
...
def test_categorise_multiple_truthiness_values(database, setup_test_database):
    """Test truthiness of a Value from a filtered value"""
    input_data = [
        *patient(1, event("abc"), positive_test(True)),
        *patient(2, event("xyz"), positive_test(False)),
        *patient(3, event("abc"), positive_test(False)),
        *patient(4, event("def"), positive_test(True)),
    ]
    setup_test_database(input_data)
...
def test_age_as_of(database, setup_test_database):
    input_data = [
        *patient(1, event("abc", "2020-10-01"), dob="1990-8-10"),
        *patient(2, event("abc", "2018-02-01"), dob="2000-03-20"),
    ]
    setup_test_database(input_data)
